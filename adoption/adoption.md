...
| --- | --- | --- |
| **Location** | `~/best-practices` | Predictable path across all projects |
...
| Step | Action |
| --- | --- |
| **1. Copy template** | `cp ~/best-practices/adoption/template-agents.md ./AGENTS.md` |
| **2. Symlink for Claude** | `ln -s AGENTS.md CLAUDE.md` |
| **3. Fill project sections** | Tech stack, commands, project-specific boundaries |
| **4. Verify reference path** | Ensure `~/best-practices` path is correct for your setup |
...
| **Has AGENTS.md with local standards** | Migrate local standards to best-practices or mark as intentional overrides |

---

## The Reference Directive

The core mechanism is a directive in your project's AGENTS.md that tells AI to consult best-practices.

```markdown
## Standards Reference

This project follows organizational standards defined in `~/best-practices/`.

**Before implementing**, consult the relevant guide:

| Topic | Guide |
| --- | --- |
| Error handling | `~/best-practices/guides/error-handling/error-handling.md` |
| Logging | `~/best-practices/guides/logging-practices/logging-practices.md` |
| API design | `~/best-practices/guides/api-design/api-design.md` |
| Documentation | `~/best-practices/guides/documentation-guidelines/documentation-guidelines.md` |
| Code style | `~/best-practices/guides/coding-guidelines/coding-guidelines.md` |

For other topics, check `~/best-practices/README.md` for the full guide index (all guides are in `~/best-practices/guides/`).
...
Copy `~/best-practices/adoption/template-agents.md` to your project as `AGENTS.md`.
...
| **Standards Reference** | Usually unchanged; update path if not using `~/best-practices` |
...
<!-- Any deviation from ~/best-practices must be documented here with rationale -->
...
1. Run `git -C ~/best-practices pull` to update standards
2. Review recent changes: `git -C ~/best-practices log --oneline -20`
...
**Effect**: AI asks before doing something that contradicts best-practices.
...
When best-practices is updated:

```bash
cd ~/best-practices && git pull
```
...
| --- | --- |
| **Useful to other projects** | Add to best-practices, not project AGENTS.md |
...
| Should this be standard? | Propose change to best-practices |
...
| AI can't find guides | Wrong path | Verify `~/best-practices` exists and path is correct |
...
| Standards seem outdated | Haven't pulled recently | Run `git -C ~/best-practices pull` |

### Verifying Setup

```bash
# Check best-practices is cloned
ls ~/best-practices/README.md
