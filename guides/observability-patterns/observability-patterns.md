...
> **Scope**: Applies to any production system. Extends beyond logging to include metrics, distributed tracing, and alerting. Systems must be observable by default.
...
| **High-cardinality metrics** | Metric explosion, cost | Use labels with bounded values |
| **Missing trace context** | Can't follow requests | Propagate context headers |
| **Sensitive data in telemetry** | Privacy/security risk | Sanitize before emitting |
| **Alerting on causes** | Miss novel failure modes | Alert on symptoms (Golden Signals) |
